...
class SAEncoder(nn.Module):
    def __init__(self, dim=512, hidden_dim=512, num_heads=1):
        ...
        self.audio_attn_mat = nn.MultiheadAttention(self.dim, self.num_heads)
        
    def forward(self, video_embd, audio_embd, text_embd, mask):
        if self.training:
            video_size = video_embd.size()
            audio_size = audio_embd.size()
            text_size = text_embd.size()
        
            video_embd = video_embd.view(-1, video_embd.size(-2), video_embd.size(-1)) #[256, 98, 512]

            audio_embd = audio_embd.view(-1, audio_embd.size(-2), audio_embd.size(-1)) #[256, 98, 512]
            ...
            tmp_audio_embd = audio_embd.permute(1, 0, 2) #[98, 256, 512]
            #print('tmp_video_embd',tmp_video_embd.shape)
            audio_embd_att, aud_attn_scores = self.vis_attn_mat(tmp_audio_embd, tmp_audio_embd, tmp_audio_embd)
            #print('video_embd_att',video_embd_att.shape) #[98, 256, 512]
            audio_embd_att = audio_embd_att.permute(1, 0, 2) #[256, 98, 512]
            #print('video_embd_att',video_embd_att.shape)
        ...
        return video_embd_att, audio_embd_att, text_embd_att, vis_attn_scores, text_attn_scores
...
def cross_attention(video_embd, text_embd, mask, train=True):
    ...
    if len(video_embd.size()) == 3:
        ...
        scores = scores / th.sqrt(th.tensor(video_embd.size(-1)).float()) #normalize
        ...
        #print('text_embd_att',text_embd_att.shape)
        text_embd_att = text_embd_att.repeat(1, text_embd.size(0), text_embd.size(1), 1, 1) #[16, 16, 20, 98, 512]
        #print('text_embd_att',text_embd_att.shape)
        text_embd_att = text_embd_att * text_att_scores.unsqueeze(-1) #[16, 16, 20, 98, 512] visual context feature for text
        #print('text_embd_att',text_embd_att.shape)
        text_embd_att = text_embd_att.sum(-2) #[16, 16, 20, 98, 512]
        #print('text_embd_att',text_embd_att.shape)

        # Computes region-attended word representations
        vis_att_scores = scores.permute(0, 1, 3, 2) #[16, 16, 98, 20]
...
class S3D_all(nn.Module):

    def __init__(self, num_classes=512, gating=True, space_to_depth=False,
                  word2vec_path='', init='uniform', token_to_word_path='data/dict.npy',global_feature=False):
        super(S3D_all, self).__init__()
        ...
        self.DAVEnet_fc = nn.Linear(1024, num_classes)
        ...
        if global_feature:
            ...
            self.DAVEnet_fc_g = nn.Linear(1024, num_classes)
    ...
    def forward(self, video, audio_input, text, mask, mode='all', mixed5c=False, att=False):
      if mode == 'all':  
          ...
          dave_out = self.DAVEnet(audio_input)#.mean(dim=2) #[B,1024,60]
          dave_out = dave_out.permute(0, 2, 1) #[B,60,1024]
          audio_embd = self.DAVEnet_fc(dave_out)
          #print('audio_embd',audio_embd.shape)
          audio_embd_att, _, vis_scores, text_scores = self.ca_layer1(audio_embd, text_embd, mask)
          #print('audio_embd_att',audio_embd_att.shape)

          video_embd_att, audio_embd_att, text_embd_att, _, _ = \
              self.sa_layer(video_embd_att, audio_embd_att, text_embd_att, mask) #[16, 16, 98, 512]
          #print('audio_embd_att',audio_embd_att.shape)
          #print('video_embd_att',video_embd_att)
          #print('text_embd_att',text_embd_att)
          #print('video_embd_att',video_embd_att.shape,'text_embd_att',text_embd_att.shape)
          ...
          final_audio_embd_att, _, _, _ = self.ca_layer2(audio_embd_att, text_embd_att, mask) #[16, 16, 98, 512]
          ...
          #print('final_audio_embd_att',final_audio_embd_att.shape)
          final_audio_embd_att = th.mean(final_audio_embd_att,-2)
          #print('final_audio_embd_att',final_audio_embd_att.shape)
          #dave_out = self.DAVEnet(audio_input)#.mean(dim=2) #[B,1024,60]
          #print('dave_out',dave_out.shape)
          
            #final_text_embd_att = th.mean(final_text_embd_att,1)


          #print('final_video_embd_att',final_video_embd_att)
          #print('final_text_embd_att',final_text_embd_att)
          return final_video_embd_att, final_audio_embd_att, final_text_embd_att
          ...
          # shouldn't project again? should have two layers
          # this is fc2
          ...
          dave_out = self.DAVEnet(audio_input)#.mean(dim=2) #[B,1024,60]
          dave_out_g = dave_out.mean(dim=2)
          dave_out = dave_out.permute(0, 2, 1) #[B,60,1024]
          audio_embd = self.DAVEnet_fc(dave_out)
          audio_embd_g = self.DAVEnet_fc_g(dave_out_g)
          ...
          final_text_embd_att = final_text_embd_att.sum(-2)
          final_text_embd_att = final_text_embd_att / mask.sum(-1).unsqueeze(-1).float()
          final_video_embd_att = final_video_embd_att.mean(-2) #[16, 16, 98, 512] => [16, 16,  512]

          final_audio_embd_att = th.mean(final_audio_embd_att,-2)

          return final_video_embd_att, final_audio_embd_att, final_text_embd_att,\
              video_embd_g, audio_embd_g, text_embd_g #single embedding for each instance
          ...
          return final_video_embd_att, final_audio_embd_att, final_text_embd_att,\
              video_embd_g, audio_embd_g, text_embd_g, final_video_embd_att_region
